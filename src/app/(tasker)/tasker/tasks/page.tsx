...
export default function TaskerTasksPage() {
  ...
  const [status, setStatus] = useState("Login as Tasker, then browse open tasks and place bids here.");
  ...
  const loadTasks = useCallback(async (currentSession: Session) => {
    ...
    setBusy("refresh");
    const response = await tasksApi.browseTasks(readBaseUrl(), currentSession.token);
    setBusy(null);
  ...
  useEffect(() => {
    const taskerSession = readSessions().tasker;
    setSession(taskerSession);
    if (taskerSession) {
      void loadTasks(taskerSession);
    }
  }, [loadTasks]);
  ...
  async function placeBid(event: FormEvent<HTMLFormElement>) {
    ...
    if (!session) return;
    setBusy("bid");
    const response = await bidsApi.placeBid(readBaseUrl(), session.token, Number(taskId), {
      ...
      <div className="space-y-6">
        <section className="rounded-lg border border-brand-100 bg-white p-6 shadow-soft">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-brand-700">Tasker marketplace</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-normal text-ink">Find work worth bidding on</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted">
                Browse available jobs, select the right opportunity, and send a proposal that helps the client choose you.
              </p>
            </div>
        ...
        <StatusBox
          ...
          message={session ? status : "Login as Tasker to load live posted tasks and submit bids."}
        />
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-muted">
                  Task listings are protected by your API, so sign in as Tasker to see real posted jobs.
                </p>
              </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
          <section>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {tasks.length === 0 ? (
                <StatusBox message="No posted tasks are available yet. When clients publish jobs, they will appear here." />
              ) : (
                tasks.map((task) => (
                  <TaskCard
                    key={task.id}
